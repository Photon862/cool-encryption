"""
ENCODE MESSAGE - Run this to encode your message using binary sound encryption
"""
...
def log_action(action_type, message, tone_duration=None, freq_min=None, freq_max=None, freq_increment=None):
    """Log action to log.csv with timestamp and settings."""
    ...
    # Format log entry
    log_entry = f'ENCODED, {message}, ToneDuration={tone_duration}, {freq_min}-{freq_max}HZ, Increment={freq_increment}, Binary'
...
if __name__ == '__main__':
    try:
        ...
        print(f"\n{e}")
        print("\nTo generate the key, run: python generate_map.py")
        sys.exit(1)
