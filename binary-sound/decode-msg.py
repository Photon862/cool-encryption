...
decoder = Decoder()
decoded_msg = decoder.decode(f'{OUTPUT_DIR}/test.wav')
print(f"✓ Decoded: {decoded_msg}")

# Log in 1 line
csv.writer(open(f'{OUTPUT_DIR}/log.csv', 'a', newline='', encoding='utf-8')).writerow([datetime.now().strftime('%Y-%m-%d %H:%M:%S'), "DECODED", decoded_msg])
