...
class Decoder:
    ...
    def decode(self, wav_file):
        """
        Decode a WAV file back to text.
        
        Example:
            decoder = Decoder()
            text = decoder.decode("output/message.wav")
            print(text)  # May be scrambled like "HELO" for "HELLO"
        
        Args:
            wav_file: Path to WAV file
        
        Returns:
            list: Decoded words (order within word may be scrambled)
        """
        # Load WAV file
        ...
        # Decode chunks
        words = decode_audio_chunks(audio_chunks, self.freq_map)
        return words
    
    def _load_wav(self, filepath):
        """
        Load WAV file and split into chunks (one per word).
        """
        ...
        # Split into chunks (one per word)
    ...
    def show_details(self, wav_file):
        ...
        words = self.decode(wav_file)
        
        print(f"\n{'='*60}")
        print(f"DETAILED ANALYSIS: {wav_file}")
        print(f"{'='*60}")
        
        for word_idx, chunk in enumerate(chunks, 1):
            print(f"\nWord {word_idx}: {words[word_idx-1] if word_idx <= len(words) else '?'}")
            print("-" * 40)
            ...
            for freq_idx, freq in enumerate(detected_freqs, 1):
                ...
                chars.append((freq, char))
                print(f"    {freq_idx}. {freq:8.1f} Hz -> '{char}'")
        
        print(f"\n{'='*60}")
        print(f"DECODED WORDS: {' | '.join(words)}")
        print(f"{'='*60}\n")
